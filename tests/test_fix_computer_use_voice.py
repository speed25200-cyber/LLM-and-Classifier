...
A = list(ACTIONS)
TR = list(GUARDRAILS["tool_risk"]["criteria"])   # readonly, workspace_write, destructive, privileged, exfiltration
SAFE = {"tool_risk": [0.9, 0.04, 0.02, 0.02, 0.02], "risk": [0.9, 0.06, 0.02, 0.02], "policy_violation": [0.05, 0.95]}
RISKY = {"tool_risk": [0.05, 0.0, 0.85, 0.05, 0.05], "risk": [0.05, 0.1, 0.25, 0.6], "policy_violation": [0.3, 0.7]}
...
def test_step_guard_counts_the_risky_probability_mass_not_only_the_argmax():
    """Garde par pas alignee sur le juge de Prophet : readonly en tete (0.40) mais 60 % de masse risquee -> confirmation."""
    from jev_clone.computer_use import StepGuard
    table = {"tool_risk": [0.40, 0.0, 0.3, 0.15, 0.15], "risk": [0.4, 0.5, 0.1, 0.0], "policy_violation": [0.1, 0.9]}
    s1 = SystemOneEngine(ScriptedBackend([table], declared={"tool_risk": TR}), model_name="mock")
    asked = []
    guard = StepGuard(s1, confirm=lambda d, j: asked.append(j) or False, kind="desktop")
    r = guard.check("vider le dossier data", {"action": "press_keys", "keys": "ctrl+a del"}, None)
    assert r["judged"]["tool_risk"] == "readonly" and r["judged"]["p_risky"] >= 0.5
    assert r["judged"]["needs_confirmation"] and asked and not r["allowed"]
