...
class Workspace:
    ...
    def memory(self, last: int = 20) -> list[str]:
        if not self.memory_file.exists():
            return []
        return [json.loads(l)["note"] for l in open(self.memory_file) if l.strip()][-last:]
...
class Prophet:
    ...
    def _guarded(self, request: str, calls: list[dict], kind: str, describe: str, action: Callable[[], dict], record: dict) -> dict:
        judged = self.judge(request, describe)
        if judged["needs_confirmation"] and not self.confirm(describe, judged):
            calls.append({"tool": kind, **record, "blocked": True, "judged": judged})
            return {"ok": False, "blocked": True, "reason": "needs user confirmation and was not confirmed", "judged": judged}
        r = action()
        calls.append({"tool": kind, **record, "ok": r.get("ok"), "judged": judged})
        return r
    ...
    def _catalog(self, request: str, calls: list[dict]) -> dict[str, tuple[dict, Callable]]:
        ...
        def python(a):
            ...
            return self._guarded(request, calls, "python", f"python code:\n{code[:1500]}",
                                 lambda: ws.run(f"{sys.executable} - <<'PROPHET_EOF'\n{code}\nPROPHET_EOF", timeout=self.command_timeout), {"code": code[:200]})
        def create_tool(a):
        ...
        cat = {
            ...
            "read_file": (_tool("read_file", "Read a text file from the workspace.", {"path": {"type": "string"}}, ["path"]), lambda a: ws.read(a["path"])),
            "list_files": (_tool("list_files", "List files under a workspace directory.", {"path": {"type": "string"}}, []), lambda a: ws.listing(a.get("path", "."))),
            "run_command": (_tool("run_command", "Run a shell command in the workspace (tests, builds, installs, running apps).", {"command": {"type": "string"}}, ["command"]), run_command),
    ...
    def handle(self, request: str, history: list[dict] | None = None) -> Turn:
        ...
        system = SYSTEM_PROMPT.format(workspace=self.ws.root, memory=("\nWhat you remember about the user:\n- " + "\n- ".join(memory)) if memory else "")
        calls: list[dict] = []

        if pre["direct"]["noul"] >= 0.8 and pre["needs_reasoning"]["noul"] < 0.5 and risk_level == 0:
            a = self.s2.chat([{"role": "system", "content": system}, *history[-6:], {"role": "user", "content": request}],
                             max_tokens=600, thinking_budget=0, temperature=0.5)["choices"][0]["message"].get("content", "").strip()
            turn = Turn(request, pre, "direct", a)
        else:
            catalog = self._catalog(request, calls)
            exposed = self._select_tools(request, catalog, pre)
            tools = {n: catalog[n] for n in exposed}
            loop = AgentLoop(self.s2, self.toolbox, tools, max_turns=self.max_turns, thinking_budget=budget, max_tokens=4096)
