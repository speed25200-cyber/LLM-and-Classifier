...
class StepGuard:
    ...
    def judge(self, goal: str, action: dict, state: PageState | None) -> dict:
        ...
        a = r.answers
        j = {"tool_risk": a["tool_risk"].choice, "tool_risk_conf": a["tool_risk"].confidence, "risk": a["risk"].score,
             "policy_violation": a["policy_violation"].noul}
        j["needs_confirmation"] = (j["tool_risk"] in ("destructive", "privileged", "exfiltration") or round(j["risk"]) >= self.risk_level
                                   or j["policy_violation"] >= 0.5)
