...
| `docs/11-JARVIS.md`, `jev_clone/jarvis.py` | **Jarvis** : assistant local a deux vitesses (REPL `jev jarvis`), cree des applications, execute des commandes sous garde-fou, memoire, journal ; `training/seeds/` + `make_synthetic_jarvis.py` : le jeu de donnees fabrique a partir de zero |
...
| `tests/` | 55 tests (hors ligne, navigateur Playwright, banc Jev, conforme, integration contre un `llama-server` reel) |
